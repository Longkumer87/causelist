...
if (!$result) {
    echo "<script>alert('Fetch Error'); window.history.back();</script>";
    exit();
}
...
<nav class="navbar flex-wrap d-flex justify-content-between px-3 shadow-sm no-print" style="background-color:#2f3e46;">
    ...
    <div class="d-flex gap-2">
        ...
        <a href="generate_doc.php?cause_date=<?= $date ?>" class="btn btn-outline-primary btn-sm">
...
<?php else: ?>
    ...
    <?php if (!empty($note)): ?>
        <div class="container-fluid px-4 mt-2">
           <p class="mb-0"><strong><?= nl2br(htmlspecialchars($note)); ?></strong></p>
        </div>
    <?php endif; ?>
